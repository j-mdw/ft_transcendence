...
export default Vue.extend({
  ...
  methods: {
    ...
    async becomeFriends (peerId: string) {
      try {
        await this.$axios.$post(`relationships/${peerId}`, { type: RelationshipType.friend }, { withCredentials: true })
      } catch {
    ...
    async deleteFriends (peerId:string) {
      try {
        await this.$axios.$delete(`relationships/${peerId}`, { withCredentials: true })
      } catch {
        this.alertBlocked = true;
      }
    },
